...
import { ActionTypes, Booking, Facility, FACILITY_ACTIONS, userCCA } from './types'
import { ENDPOINTS, DOMAINS, DOMAIN_URL, put } from '../endpoints'
import dayjs from 'dayjs'
import axios from 'axios'

export const SetCreateBookingError =
  (newError: string) =>
...
export const getAllBookingsForFacility =
  (ViewStartDate: Date, ViewEndDate: Date, selectedFacilityId: number) =>
  async (dispatch: Dispatch<ActionTypes>): Promise<void> => {
...
export const setIsDeleteMyBooking =
 ...
 * @returns
 */
export const deleteMyBooking =
  (bookingId: number) =>
  async (dispatch: Dispatch<ActionTypes>, getState: GetState): Promise<void> => {
    const TokenId = localStorage.getItem('token')
...
export const checkForDurationError =
...
export const setViewDates =
  (newDate: Date, selectedFacilityId: number) =>
  (dispatch: Dispatch<ActionTypes>): void => {
...
export const setViewFacilityMode =
  (currentMode: boolean) =>
  (dispatch: Dispatch<ActionTypes>): void => {
...
export const createNewBookingFromFacility =
  (startDate: Date, endDate: Date, facilityName: string, facilityId: string) =>
  (dispatch: Dispatch<ActionTypes>): void => {
...
export const setNewBookingFacilityName =
  (name: string) =>
  (dispatch: Dispatch<ActionTypes>): void => {
    dispatch({ type: FACILITY_ACTIONS.SET_BOOKING_FACILITY, newBookingFacilityName: name })
  }

export const fetchAllCCAs =
  () =>
  async (dispatch: Dispatch<ActionTypes>): Promise<void> => {
    const { data: ccaList } = await axios.get<userCCA[]>(DOMAINS.EVENT + ENDPOINTS.ALL_CCAS)

    dispatch({ type: FACILITY_ACTIONS.GET_ALL_CCA, ccaList })
    dispatch(SetIsLoading(false))
  }

export const fetchFacilityNameFromID =
  (id: number) =>
  async (dispatch: Dispatch<ActionTypes>): Promise<void> => {
...
export const resetCreateBookingSuccessFailure =
  (failureBoolean: boolean, successBoolean: boolean) =>
  (dispatch: Dispatch<ActionTypes>): void => {
    dispatch({
  ...
  async (dispatch: Dispatch<ActionTypes>, getState: GetState): Promise<void> => {
    ...
    if (!isEdit) {
      ...
      if (response.status >= 400) {
        ...
        dispatch({
          type: FACILITY_ACTIONS.SET_BOOKING_FACILITY_ID,
          newBookingFacilityId: selectedFacilityId.toString(),
        })
...
export const fetchSelectedFacility =
  (bookingId: number) =>
  async (dispatch: Dispatch<ActionTypes>): Promise<void> => {
