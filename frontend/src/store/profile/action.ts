...
export const fetchAllCCAs =
  ...
  async (dispatch: Dispatch<ActionTypes>): Promise<void> => {
    dispatch(setIsLoading(true))
    const { data } = await axios.get<{ data: UserCCA[] }>(DOMAIN_URL.EVENT + ENDPOINTS.ALL_CCAS)
...
export const populateProfileEdits =
  () =>
...
export const handleModuleDetails =
  (actionType: 'Delete' | 'Add', newModule: string) =>
  (dispatch: Dispatch<ActionTypes>, getState: GetState): void => {
...
export const handleNewProfilePicture =
