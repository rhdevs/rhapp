...
import type { Position, Post } from './types'
...
export const getUserDetail =
  ...
  async (dispatch: Dispatch<ActionTypes>): Promise<void> => {
    // Get the token and send a GET request to the server
    ...
    >(DOMAINS.SOCIAL + ENDPOINTS.USER + '/' + userID)

    // Dispatch the data
    dispatch({
      type: SOCIAL_ACTIONS.GET_USER_DETAIL,
      userId: data[0].userID,
      avatar: data[0].profilePictureUrl,
      name: data[0].displayName,
      position: data[0].position,
      telegramHandle: data[0].telegramHandle,
    })
  }

export const GetPostDetailsToEdit =
  () =>
...
export const handleEditPost =
  ...
  async (dispatch: Dispatch<ActionTypes>, getState: GetState): Promise<void> => {
    ...
    await axios.put(DOMAINS.SOCIAL + ENDPOINTS.EDIT_POST, requestBody)
...
export const handleCreatePost =
  ...
  async (dispatch: Dispatch<ActionTypes>, getState: GetState): Promise<void> => {
    ...
    await axios.post(DOMAINS.SOCIAL + ENDPOINTS.CREATE_POSTS, requestBody)
    dispatch(GetPosts(POSTS_FILTER.ALL))
    ...
    get(endpoint, DOMAINS.SOCIAL, subroute).then((response) => {
      if (response.data.length > 0) {
        ...
        if (postDiff.length > 0) {
          const diffTransformedPosts: Post[] = transformedPost.filter((post) => postDiff.includes(post.postId))
...
export const GetSpecificPost =
  ...
  async (dispatch: Dispatch<ActionTypes>): Promise<void> => {
    ...
    }>(`${DOMAIN_URL.SOCIAL}${ENDPOINTS.SPECIFIC_POST}?postID=${postId}`)
    const specificPost = response.data.data

    const { postID, title, createdAt, ccaID, isOfficial, description, postPics, name, userID, profilePic } =
      specificPost
    const newPost = {
      name: name,
      userId: userID,
...
export const SetHasNoMorePosts =
